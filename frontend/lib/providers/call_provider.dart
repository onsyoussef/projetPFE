...
class CallProvider extends ChangeNotifier {
  CallProvider({
    ...
    _webrtc.callRejected.listen((_) {
      _setState(CallState.refuse);
      _stopTimer();
    });
    _webrtc.callEnded.listen((_) {
      _setState(CallState.termine);
      _stopTimer();
    });
    _webrtc.connectionStates.listen((s) {
      connectionState = s;
  ...
  Future<bool> acceptIncomingCall({
    ...
    try {
      ...
      debugPrint('[CALL][frontend] acceptIncomingCall error: $e');
      debugPrint('$st');
      _setState(CallState.echec);
      return false;
    }
  ...
  void rejectIncomingCall(String fromUserId, {String? callRoomId}) {
    _webrtc.rejectCall(toUserId: fromUserId, roomId: callRoomId ?? roomId);
    _setState(CallState.refuse);
  }
