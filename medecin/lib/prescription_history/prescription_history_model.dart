...
class PrescriptionHistoryEntry {
  const PrescriptionHistoryEntry({
    required this.id,
    required this.prescriptionMessageId,
  ...
  factory PrescriptionHistoryEntry.fromJson(Map<String, dynamic> json) {
    ...
    return PrescriptionHistoryEntry(
      id: '${json['id'] ?? ''}'.trim(),
      prescriptionMessageId:
          '${json['prescriptionMessageId'] ?? json['messageId'] ?? ''}'.trim(),
      ...
      doctorName: readableDoctorName(json['doctorName']?.toString(), fallback: ''),
